...
import { BlogsNewsdata, clientData } from '../../Data/data';

export default function BlogDetail(props) {
    const blog = BlogsNewsdata.find((projets) => projets?.id === parseInt(props?.params?.id ||0));
    const client = clientData.find((clients) => clients.id === parseInt(props?.params?.clientId ||0));
    return (
        <>
            <Navbar />
            <section className="py-28 w-full table relative bg-[url('/images/bg/bg-5.jpg')] bg-bottom bg-no-repeat" id="home">
                ...
                <div className="container">
                    <div className="grid grid-cols-1 pb-8 text-center mt-10">
                        <h3 className="mb-3 font-medium leading-normal text-3xl mt-10 text-white">{blog?.title}</h3>
                        ...
                        <ul className="list-none mt-6">
                            ...
                            <li className="inline-block text-white/50 mx-5"> <span className="text-white block">Date :</span> <span className="block">15 Aout 2022</span></li>
                            <li className="inline-block text-white/50 mx-5"> <span className="text-white block">Adresse :</span> <span className="block">Tizi Ouzou</span></li>
                        </ul>
                    </div>
                </div>
            </section>
            <section className="relative md:py-24 py-16">
                    ...
                    <div className="grid md:grid-cols-12 grid-cols-1 gap-[30px]">
                        <div className="lg:col-span-8 md:col-span-6">
                            ...
                            <div className="p-6 rounded-md shadow dark:shadow-gray-800 mt-8">
                                <h5 className="text-lg font-semibold">Laissez un commentaire:</h5>
                                ...
                                <form className="mt-8">
                                        ...
                                        <div className="lg:col-span-6 mb-5">
                                            <div className="text-start">
                                                ...
                                                <div className="form-icon relative mt-2">
                                                    ...
                                                    <input name="name" id="name" type="text" className="form-input w-full py-2 px-3 ps-11 bg-transparent border border-inherit dark:border-gray-800 dark:bg-slate-900 dark:text-slate-200 rounded outline-none focus:border-violet-600/50 dark:focus:border-violet-600/50 focus:ring-0" placeholder="Nom :" />
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div className="grid grid-cols-1">
                                        <div className="mb-5">
                                            <div className="text-start">
                                                <label htmlFor="comments" className="font-semibold">Votre commentaire:</label>
                                                <div className="form-icon relative mt-2">
                                    ...
                                    <button type="submit" id="submit" name="send" className="py-2 px-5 inline-block font-normal tracking-wide border align-middle transition duration-500 ease-in-out text-base text-center bg-violet-600 hover:bg-violet-700 border-violet-600 hover:border-violet-700 text-white rounded-md w-full">Envoyer</button>
                                </form>
                            </div>
                        </div>
                        ...
                        <div className="lg:col-span-4 md:col-span-6">
                            <div className="sticky top-20">
                                <h5 className="text-lg font-medium bg-gray-50 dark:bg-slate-800 shadow dark:shadow-gray-800 rounded-md p-2 text-center">Auteur</h5>
                                <div className="text-center mt-8">
                                    <Image src="/images/client/05.jpg" className="h-24 w-24 mx-auto rounded-full shadow mb-4" alt="" width={96} height={96}/>

                                     <Link href="" className="text-lg font-medium hover:text-violet-600 transition-all duration-500 ease-in-out h5">{client?.name}</Link>
                                    <p className="text-slate-400">CEO</p>
                                </div>
                                <h5 className="text-lg font-medium bg-gray-50 dark:bg-slate-800 shadow dark:shadow-gray-800 rounded-md p-2 text-center mt-8">Investissements</h5>
                                    <div className="grid grid-cols-3 gap-6">
                                        <p className={blog?.state === "En cours" ? "text-violet-600 bg-teal-200 text-center rounded-md" : "text-gray-300 bg-orange-400 text-center rounded-md"} >{blog?.state}</p>
                                        <p className="text-violet-600">{blog?.actions} Actions</p>
                                        <p className="text-violet-600">{blog?.price}</p>
                                    </div>
                                    ...
                                    <div className="grid lg:grid-cols-12 lg:gap-6">
                                        <div className="lg:col-span-12 mb-5">
                                            <div className="text-center">
                                                <label htmlFor="name" className="font-semibold">Prix DA</label>
                                                <div className="form-icon relative mt-2">
                                        
                                                    <input name="name" id="name" type="text" className="form-input w-full py-2  text-center bg-transparent border border-inherit dark:border-gray-800 dark:bg-slate-900 dark:text-slate-200 rounded outline-none focus:border-violet-600/50 dark:focus:border-violet-600/50 focus:ring-0" placeholder="10000" />
                                                </div>
                                            </div>
                                        </div>
                                    </div>   
                                    <button type="submit" id="submit" name="send" className="py-2 px-5 inline-block font-normal tracking-wide border align-middle transition duration-500 ease-in-out text-base text-center bg-violet-600 hover:bg-violet-700 border-violet-600 hover:border-violet-700 text-white rounded-md w-full">Investir</button>
                                
                                    </form>


                                <h5 className="text-lg font-medium bg-gray-50 dark:bg-slate-800 shadow dark:shadow-gray-800 rounded-md p-2 text-center mt-8">Sites sociaux</h5>
                ...
                <BlogsNews title="Projets liés"/>
                <div className="container">
                    <div className="grid grid-cols-1 text-center">
                        <h3 className="mb-6 md:text-2xl text-xl font-medium">Vous avez des questions ? Entrer en contact!</h3>

                        <p className="text-slate-400 max-w-xl mx-auto">This is just a simple text made for this unique and awesome template, you can replace it with any text.</p>

                        <div className="mt-6">
                             <Link href="/contactus" className="py-2 px-5  font-normal tracking-wide border align-middle transition duration-500 ease-in-out text-base text-center bg-violet-600 hover:bg-violet-700 border-violet-600 hover:border-violet-700 text-white rounded-md inline-flex items-center gap-1"><Unicons.UilPhone width={16}/> Contactez nous</Link>
                        </div>
                    </div>
                </div>
            </section>
