                        ...
                        return (
                            ...
                            <div className="blog relative rounded-md shadow shadow-slate-200 dark:shadow-gray-800 overflow-hidden" key={index}>
                                ...
                                <div className="content p-6">
                                     <Link href={`/projet-detail/${item.id}`} className="text-lg hover:text-violet-600 dark:text-white dark:hover:text-violet-600 transition duration-500 ease-in-out font-medium h5">{item.title}</Link>
                                    <p className="text-slate-400 mt-3">{item.detail}</p>
                                    <div className="grid grid-cols-2 gap-6">
                                        <p className={item.state === "En cours" ? "text-violet-600 bg-teal-200 text-center rounded-md" : "text-gray-300 bg-orange-400 text-center rounded-md"} >{item.state}</p>
