...
export default function NavbarSmallLight() {
   ...
   useEffect(() => {
    ...
    return (
        <>
            <nav className={`${navbarTop === true ? 'is-sticky' : ''} navbar font-Arial `}id="navbar">
                <div className="container flex flex-wrap items-center justify-end">
                     <a className="navbar-brand md:me-8" href="/">
                        <Image src="/images/logo-dark copy.png" className="inline-block dark:hidden" alt="" width={113} height={28}/>
                        <Image src="/images/logo-light copy.png" className="hidden dark:inline-block" alt="" width={113} height={28}/>
                    </a>
                    ...
                    <div className={`${isOpen === true ? 'navigation lg_992:order-1 lg_992:flex hidden ms-auto' : 'navigation lg_992:order-1 lg_992:flex block ms-auto'}`} id="menu-collapse">
                        <ul className="navbar-nav" id="navbar-navlist">
                            <li className={`nav-item`}>
                                <Link1 to="home" activeClass="active" spy={true} smooth={true} duration={500} className="nav-link">Accueil</Link1>
                            </li>
                            <li className={`nav-item`}>
                                <Link1 to="features" activeClass="active" spy={true} smooth={true} duration={500} className="nav-link">Services</Link1>
                            </li>
                            <li className={`nav-item`}>
                                <Link1 to="pricing" activeClass="active" spy={true} smooth={true} duration={500} className="nav-link">Tarifs</Link1>
                            </li>
                            <li className={`nav-item`}>
                                <Link1 to="testi" activeClass="active" spy={true} smooth={true} duration={500} className="nav-link">Revoir</Link1>
                            </li>
                            <li className={`nav-item`}>
                                <Link1 to="blog" spy={true} smooth={true} duration={500} className="nav-link">Projets</Link1>
                            </li>
                            <li className={`nav-item`}>
                                <Link1 to="contact" activeClass="active" spy={true} smooth={true} duration={500} className="nav-link">Contactez nous</Link1>
                            </li>
                        </ul>
                    </div>
                </div>
